...
//{"txt_codigo_periodo":"","txt_nombre_periodo":"oswaldo","txt_rango_fecha":"01/07/2016 - 01/09/2016","txt_fecha_inicio":"","txt_fecha_cierre":"","txt_radio_estatus":"0","accion":"agregar_permisos"}
	...
	switch ($accion){
		...
		break;
		case 'procesar_permisos':
			procesar_permisos($datos);
		break;
...
<?php
	function consultar_permisos($datos) {
		...
		if ($persona_cedula != "") {
			...
			// ver_arreglo($cadena_cedula);
			// ver_arreglo($persona_cedula);
			// ver_arreglo("sin cedula");
		} 
		...
		$sql= "SELECT 	P.id_permiso, 
			...
			// $sql.= "	ORDER BY P.fecha_recepcion DESC, P.estatus ASC";
...
<?php
	function agregar_permisos($datos) {
		...
		$estatus = '4'; // 1 Pendiente (Secretaria, Juridica) 2 Aprobado, 3 Rechazado, 4 Aprob Jefe Inmediato, 5 Aprob Jefe Personal  
...
?>
<?php
	function procesar_permisos($datos) {
		// PARAMETROS DEL FORMULARIO
		$id_permiso	=	$_POST['id_permiso']; // 1
		$txt_estatus			=	$_POST['txt_estatus']; // 1
		$txt_observacion		=	$_POST['txt_observacion']; // 0
		...
		$sql ="	UPDATE permisos.permisos
						SET 	estatus='$txt_estatus', 
					       		estatus_observacion='$txt_observacion'
